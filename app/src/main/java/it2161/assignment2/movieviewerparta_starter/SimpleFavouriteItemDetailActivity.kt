...
class SimpleFavouriteItemDetailActivity : AppCompatActivity() {
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.removefavourite, menu)
        return true
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.removefavourite -> {
                val mc = MovieViewerApplication.appInstance
                val addMovieTitle = movietitle.text.toString().trim()
                mc.deleteFrmFavDatabase(addMovieTitle, applicationContext)
